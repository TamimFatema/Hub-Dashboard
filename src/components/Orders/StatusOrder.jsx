...
const StatusOrder = ({ status, setOrderCount }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(url, { headers })
      .then((response) => {
        const statusOrders = response.data.filter(
          (order) => order.order_status === status
        );
        setOrders(statusOrders);
        setFilteredOrders(statusOrders);
        setOrderCount(statusOrders.length);
      })
      .catch((error) => {
        console.error("Error fetching data:", error);
        setError(error.message);
      });
  }, [status, setOrderCount]);
  ...
  //     (order) =>
  //       order.id.toString().toLowerCase().includes(term.toLowerCase()) ||
  //       order.customer.first_name.toLowerCase().includes(term.toLowerCase()) ||
  //       order.customer.mobile_number
  //         .toString()
  //         .toLowerCase()
  //         .includes(term.toLowerCase()) ||
  //       order.store.name.toLowerCase().includes(term.toLowerCase()) ||
  //       order.order_status.toLowerCase().includes(term.toLowerCase())
  //   );
  //   setFilteredOrders(filtered);
  // };

  // const debouncedSearch = useCallback(_.debounce(searchOrders, 300), [orders]);
                ...
                <tr className="border" key={order.id}>
                  ...
                  <td className="p-3">
                    <div>
                      ...
                      <p>{order.order_date.split("T")[1]}</p>
                    </div>
                  </td>
                  <td className="">
                    <div>
